...
async function fetchCompanies(diagnostics: string[]) {
  const result = await fetchBoltApi(
...
async function fetchDrivers(
  ...
  console.log(
    "BOLT DRIVERS RESPONSE:",
    JSON.stringify(result.payload, null, 2),
  );

  diagnostics.push(
    `${result.message} body=${JSON.stringify(body)}`
  );
  ...
  const drivers = parseDriversFromPayload(
    result.payload,
    companyId,
    companyName,
  );
...
async function fetchOrders(
  ...
  const orders = parseOrdersFromPayload(
    result.payload,
    companyId,
    companyName,
  );
